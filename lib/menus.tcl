...
# ::menu_edit::move_item
# delete_menu_recurse
# ::menu_edit::ask_delete_menu
# ::menu_edit::delete_item
...
set len1 [llength [split $menu .]]
if {$vTcl(menubar) && $len1 == 3} {
	set font $vTcl(actual_gui_font_dft_desc)
} else {
	set font $vTcl(actual_gui_font_menu_desc)
}
...
set len [llength [split $nmenu .]]
				# set vTcl($nmenu,font) ""
                set vTcl($nmenu,font) $vTcl(actual_gui_font_menu_name)
    ...
    proc {::menu_edit::show_menu} {top index} {
		...
		set i [lindex $reference end]
		if {![winfo exists $m]} {
			# We have the name of a cascade we have deleted so we have
            # to create menu. So we get a name for it, create it, and fill
            # in the defaults.
   ...
   set ::${top}::current_index $i  ;# NEEDS WORK Really concerned about change!
    ...
    proc keyRelease {top option var boxvar} {
        ...
        # default value
        global vTcl
		if {$option in $vTcl(color_options) && $var eq ""} {
			set $boxvar 0
    ...
    proc discoverOptions {} {
        ...
        set vTcl(options,cascade) [.m entryconfigure 0]
		set vTcl(options,cascade) [lsort $vTcl(options,cascade)]

        .m.cascade add command -label "command"
        set options [concat $options [.m.cascade entryconfigure 0]]
		set vTcl(options,command) [.m.cascade entryconfigure 0]
		set vTcl(options,command) [lsort $vTcl(options,command)]

        .m.cascade add radiobutton -label "command"
        set options [concat $options [.m.cascade entryconfigure 1]]
		set vTcl(options,radiobutton) \
			[concat $options [.m.cascade entryconfigure 1]]
		set vTcl(options,radiobutton) [lsort $vTcl(options,radiobutton)]

        .m.cascade add checkbutton -label "command"
        set options [concat $options [.m.cascade entryconfigure 2]]
		set vTcl(options,checkbutton) \
			[concat $options [.m.cascade entryconfigure 2]]
		set vTcl(options,checkbutton) [lsort $vTcl(options,checkbutton)]

        .m.cascade add separator
        set options [concat $options [.m.cascade entryconfigure 3]]
		set vTcl(options,separator) \
			[concat $options [.m.cascade entryconfigure 3]]
		set vTcl(options,separator) [lsort $vTcl(options,separator)]
	...
	proc fillProperties {top m i} {
		...
		# This is new version version for editing menus
        ...
        # For replacing "[$top.MenuCanvas]" with actual values created
        # when editor window was created.
		set scrolled_window $top.cpd24.02.scr84
		set canvas $top.cpd24.02.scr84.can85
        # First get a clean window.
		destroy $canvas.f
		frame $canvas.f
		grid $canvas.f
		$scrolled_window setwidget $canvas
        $canvas create window 0 0 -window $canvas.f \
            -anchor nw -tag properties        
		update
		
        if {$i < 0} {   # So that we can change menu properties. Rozen
            set properties [$m configure]
        } else {
            set properties [$m entryconfigure $i]
        }
		set skip_command 0
		# Want to skip the command iff we are in a cascade entry and
		# the will be th case if a menu option is present, but since
		# the menu option is likely to follow the command option. We
		# run thru the options looking for the memory option.
		foreach property $properties {
			foreach {o x y d v} $property {}
			if {$o eq "-menu"} { set skip_command 1 }
		}	
		foreach property $properties {
			foreach {o x y d v} $property {}
			if {$o eq "-menu"} continue
			if {$o eq "-command" && $skip_command} continue
			if {$o eq "-command" || $o eq "-postcommand"} {
				# Remove leading "#" characters before displaying command value.
				regsub {^[\#]+} $v "" v
			}
			if {$o in [list "-bd" "-bg" "-fg" "-cursor" \
					   "-tearoff" "-tearoffcommand" "-title" "-type"]} continue
			#################################################
			set len [llength [split $m .]]
			if {$o eq "-font" && $i >= 0 && $len == 3} {
				set v $vTcl(pr,gui_font_fixed)}
			if {$o eq "-font" && $i >= 0 && $len > 3} {
				set v $vTcl(pr,gui_font_menu)}
			#################################################
	        set vTcl(menu_prop,$o) $v
            lappend o_list $o
	        set o_list [lsort $o_list]
		}
		foreach o $o_list {
			set f $canvas.f.$o
			set variable vTcl(menu_prop,$o)
			if {[winfo exists $f]} {destroy $f}
			frame $f
			set target($top) $m
			set f $canvas.f.$o
			set config_cmd "::menu_edit::configCmd $top $o $variable"			
			::vTcl::ui::attributes::newAttribute  $target($top) $f \
                 $o $variable $config_cmd  \
                 "::menu_edit::setGetBox $top"  \
                 "::menu_edit::keyRelease $top"
			pack $f -side top -fill x -expand 0
		}
		update 
		set w [winfo width  $canvas.f]
		set h [winfo height $canvas.f]
...
return
	...
	proc configCmd {top option variable} {
		...
		if {$option in $vTcl(color_options)} {
		set color [set $variable]
	    if {$color ne "" && [catch {set rgb [winfo rgb . $color]} err]} {
			::vTcl::MessageBox -title "Color Error" -message \
				"Illegal color specification: \"$color\". Option: $option." 
		...
		# 	set properties [$m configure]
			...
			set l 0
				while {$l <= $i} {
					...
					set l_menu [lindex $reference end-1]
					set selection_v [$top.MenuListbox get $l]
					if {$l_menu == $start_menu} {
						if {![regexp {<Menu>} $selection_v]} {incr j}
					}
					incr l
				}
		    set k $j
...
proc vTclWindow.vTclMenuEdit {base menu} {
    ...
    # This is the window that has the attributes, the right hand side.
    ...
    #::menu_edit::initProperties $base $menu   ;# right hand side. No longer needed.
