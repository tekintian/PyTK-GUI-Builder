...
namespace eval ::vTcl::compounds {
      ...
      "namespace eval \{::vTcl::compounds::${type}::[list $compoundName]\} \{\n"
        ...
        if {$class == "Toplevel"} {
           ...
           append output "    set top \$target\n"
            ...
            append output \
			   "[$::classes($class,dumpCmd) $target \$target\${menu}]\n"
        } elseif {$class in "Popupmenu Menubutton TMenubutton"} {
			# New
            append output "[$::classes($class,dumpCmd) $target \$target]\n"
		} else {
			append output "[$::classes($class,dumpCmd) $target \$target]\n"
	...
	if {$mainCmd != ""} {
		...
		if {$vTcl(copy)} {
			...
			# if {$class eq "Toplevel" && [info exists vTcl(mbar_name)]} { }
			if {$class eq "Toplevel" && $vTcl(mbar_name) ne ""} {
				append output \
					"$vTcl(tab)\$top configure -menu $vTcl(mbar_name)\n"
			}
append output "# add C1\n"			
        }
        append output "\}\n\n"
        ...
        foreach child $children {
			...
			if {![info exists ::vTcl(bindtags,$child)]} {continue}
...
#"
    ...
    proc autoPlaceCompound {type compoundName gmgr gopt} {
        ...
        set namePrefix [string range $rootclass 0 2]
            ...
            # set target [vTcl:new_widget_name $rootclass $::vTcl(w,insert)]
		   
			# set target [vTcl:new_widget_name "men" $::vTcl(w,insert)]
		} ;# new location
