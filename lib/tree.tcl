...
proc vTcl:double_B1_click_tree {i X Y x y} {
    # Come here when one double clicks B1 on a widget in the tree.	
...
proc vTcl:init_wtree {{wants_destroy_handles 1}} {
	...
	foreach ii $tree {
		...
		if {$class in "Menubutton TMenubutton"} {
			set menu [$i cget -menu]
			lappend menubutton_links $menu			
		}
		if {[string first "vTH" $ii] > -1} { continue }

		# The following removes menubar from the tree when editing the
		# menu removes all entries. Another possibility whould be to
		# destroy the menu as part of ::menu_edit::delete_item.
		if {[regexp {\.m\d+$} $ii] 
			|| [regexp {\.mem\d+$} $ii]   } {
			set entries [$ii index end]
			if {$entries == "none"} {
				continue
			}
		}
		...
		if {[info exists menubutton_links] \
				&& $i in $menubutton_links} {
			continue
		}
        if {$class == "Scrollbar"} {
            continue			
        ...
        set t \
            [$b create text $x2 $y2 -text $t -anchor w  \
                 -tags "TEXT TEXT$b.$j" \
                 -fill $t_fill -font $vTcl(pr,font_dft)]
            if {[info exists vTcl(contract,$i)] && $vTcl(contract,$i)} {
				set xx [expr $x2 + $t_len]
                $b create text $xx $y2 -text " (shell)" -anchor w  \
                 -fill red -font $vTcl(pr,font_dft)
            }
